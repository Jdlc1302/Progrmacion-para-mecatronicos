...
void Lectura(producto A[], int TAM)
{
    int I;
    for (I=0; I<TAM; I++)
    {
...
void Ventas(producto A[], int TAM)
{
    int CLA, CAN, I, RES;
    float TOT, PAR;
    ...
    while(CLA)
    {
        ...
        scanf("%d", &CAN);
        I = 0;
        while ((I < TAM) && (A[I].clave < CLA))
            I++;
        if ((I == TAM)||(A[I].clave > CLA))
...
void Reabastecimiento(producto A[], int TAM)
...
void Nuevos_Productos(producto A[], int*TAM)
...
void Inventario(producto A[], int TAM)
{
    int I;
    for(I=0; I<TAM; I++)
    {
