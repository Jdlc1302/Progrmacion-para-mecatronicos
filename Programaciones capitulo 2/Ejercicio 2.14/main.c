...
/* Telefono.
El programa, al recibir como datos la clave de la zona geograficay el numero de segundos de una llamada telefonica, calcula el costo de la misma.
CLA y TIE: Variables de tipo entero.
COS: varible de tipo real. */
...
int main (void)
{
    ...
    if (COS != -1)
        printf("\n\nCave: %d\tTiempo: %d\tCosto: %6.2f", CLA, TIE, COS);
    else
        printf("\nError en la clave");
}
